...
**Live exit timing matches the backtest.** `runLive` runs `_liveSession` (entries, 09:35–12:10) and `_liveMonitorExits` (stop/trail/target/15:00) **concurrently over a shared trade list**, so each position is stop/trail-managed from the moment it fills — exactly like the backtest's per-bar exit walk from the entry bar. (Earlier they ran sequentially: exits only began after the 12:00 entry window, leaving morning trades unmanaged for hours and diverging from the backtest.) Residual edge case: a *catch-up* entry (k+1 already several bars old when the slow live scan found it) starts exit management from "now", so it can't replay a stop that already occurred between its entry bar and detection — the reconciliation diff surfaces this as an exit mismatch.
