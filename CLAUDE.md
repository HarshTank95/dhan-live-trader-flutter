...
| `ScripService` | Instrument master — fetches from Dhan API, caches to file. Loud diagnostics on failure (an empty master cripples watchlist/search/universe). Also fetches current Nifty 50/100/200/500 constituents from niftyindices.com daily (cached per-day) and feeds them to `UniverseHistory.recordCurrent` |
| `UniverseHistory` | **Point-in-time index membership** (`universe_history_service.dart`). `membersAsOf(mode, date)` answers "who was in the Nifty 50/100/200/500 on date X" from two merged sources: baked snapshots (`data/universe_history.dart`, generated by `D:\Code\tools\universe-history\build_history.js` from Wayback captures of the official NSE CSVs, symbols pre-normalized to current NSE symbols via ISIN) + self-recorded snapshots (appended to SharedPreferences whenever the daily fetch sees membership change — the app archives every future index review automatically). Removes backtest survivorship bias: May-2022 vs current list overlap is only 352/500 (~30% of the universe wrong); point-in-time recovers 139/149 dropped members (Dhan IDs persist across renames and serve pre-rename candles — probe-verified), leaving ~2% residual (truly delisted: HDFC, IDFC, …). Known gap: no nifty500 snapshot Feb-2024→Jun-2025 (uses Feb-24 membership); nifty200 pre-2023 composite is approximate |
...
**Universe mode** (`StrategyConfigModel.universeMode`, default `'static'`): `'nifty50'|'nifty100'|'nifty200'|'nifty500'` switch the strategy to point-in-time index membership — backtests fetch the superset of historical members over the range but each simulated day only scans `UniverseHistory.membersAsOf(mode, day)` (per-day filter in `_simulateDay` for dominance; the `_BtDayContext.securityIds` getter is pre-filtered for `hasCustomEngine` strategies, so they need zero universe awareness); live runs scan the current members via `ScripService.getSecurityIdsForUniverse`. **Results across different universe modes are not comparable — all pre-existing benchmarks (ORB, hammer) were validated on `static`; re-validate before trusting point-in-time numbers.**
